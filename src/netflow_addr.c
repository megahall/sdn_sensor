...
int addr_masklen_valid(int af, int masklen) {
    ...
    switch (af) {
    case AF_INET:
        return (masklen >= 0 && masklen <= 32 ? 0 : -1);
    case AF_INET6:
        return (masklen >= 0 && masklen <= 128 ? 0 : -1);
    default:
        return (-1);
    }
}
...
int addr_frame_to_xaddr(ss_frame_t* fbuf, struct xaddr* xa) {
    ...
    switch (fbuf->data.eth_type) {
        case ETHER_TYPE_IPV4: {
            xa->af = AF_INET;
            rte_memcpy(&xa->v4, &fbuf->data.sip, IPV4_ALEN);
            break;
        }
        case ETHER_TYPE_IPV6: {
            xa->af = AF_INET6;
            // XXX: mhall: bogus scope ID
            xa->scope_id = 0;
            rte_memcpy(&xa->v6, &fbuf->data.sip, IPV6_ALEN);
            break;
        }
        default: {
            return (-1);
